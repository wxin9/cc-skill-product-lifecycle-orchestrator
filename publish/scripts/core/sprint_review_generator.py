...
class SprintReviewGenerator:
    ...
    def _load_plan(self, iter_dir: Path) -> dict:
        ...
        goal_m = __import__('re').search(r'##\s*目标\s*\n+(.*)', text)
        goal = goal_m.group(1).strip() if goal_m else "（见 PLAN.md）"

        features = __import__('re').findall(r'-\s*(F\d+[^：\n]*)', text)
        e2e_m = __import__('re').search(r'E2E[^：：]*[：:]\s*\n+(.*?)(?:\n#|\Z)', text, re.DOTALL)
    ...
    def _load_test_results(self, iteration: int) -> dict:
        ...
        results = list(data.get("results", {}).values()) if isinstance(data.get("results"), dict) else []
    ...
    def _load_velocity(self) -> dict:
        ...
        if not vel_file.exists():
            return {}
        return json.loads(vel_file.read_text())
    ...
    def _render(self, iteration: int, plan: dict, tests: dict,
        ...
        act_h = current_vel.get("actual_hours", "N/A")
        vel_line = f"{est_h}h 估算 / {act_h}h 实际" if act_h != "N/A" else f"{est_h}h 估算（进行中）"
...
*本文档由 `./lifecycle gate --iteration {iteration}` 自动生成，可直接发送给 Stakeholder。*
"""
