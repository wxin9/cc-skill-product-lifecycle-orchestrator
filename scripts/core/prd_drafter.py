...
使用方式：
  ./orchestrator run --user-input "用一段话描述你的产品"

AI agent 根据描述自动生成 PRD 草案结构，用户在草案基础上审核修改，
而非从空白模板开始填写（"审稿人"模式代替"填空题"模式）。

本模块输出的是"起草提示词"（Draft Prompt）——AI agent 在 SKILL.md 中读取
...
def generate_draft_prompt(description: str, output_path: str = None) -> str:
    """
    生成 PRD 起草提示词，供 AI agent 在对话中执行。
...
AI agent 将根据你的描述生成 PRD 草案。

草案生成后，请：
  1. 阅读草案，关注 [❓待确认] 标注处
  2. 修改不准确的内容
  3. 补充空白的细节
  4. 确认后运行 ./orchestrator resume --from-phase phase-3-draft-prd
