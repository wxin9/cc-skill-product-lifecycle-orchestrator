...
使用方式：
  ./orchestrator resume --from-phase phase-7-draft-arch

读取 PRD、Product Spec、UED Spec 与架构输入，自动生成架构草案（含 ADR 初稿、API 端点草案、数据模型草案）。
...
def load_prd_summary(prd_path: str, root: str = ".") -> str:
    ...
    if not p.exists():
        return "（PRD 文件不存在，请先完成 PRD 起草和 Product Spec）"
...
def load_interview_info(root: str = ".") -> str:
    ...
    if not interview_file.exists():
        return "（架构输入未记录，请先确认 Product/UED Specs 后再起草架构）"
...
def generate_draft_prompt(root: str = ".") -> str:
    """生成架构草案提示词。"""
    from . import paths
    prd_summary = load_prd_summary(paths.PRD_PATH, root)
    ...
    try:
        ...
        print(f"[arch_drafter] project type detection failed: {e}")
...
AI agent 将根据 PRD 功能点和架构访谈信息生成架构草案。
""")
    if not has_interview:
        print("⚠️  未找到架构输入记录，建议先确认 Product/UED Specs 后再起草架构。")
        print("   请先确认 Docs/product/PRD.md、Docs/product/UED.md 和 .lifecycle/specs/ued.spec.json。\n")
    print("""草案生成后，请：
  1. 审阅技术选型，确认是否符合实际约束
  2. 检查 ADR 中的架构决策，保留合理决策并标记待确认项
  3. 补充 [❓待确认] 处
  4. 确认后运行 ./orchestrator resume --from-phase phase-7-draft-arch
